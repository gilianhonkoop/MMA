...
def recompute_lpips_for_chat(chat_id):
    with Database() as db:
        ...
        images = images[images["selected"] == 1]  # LPIPS is only meaningful between selected images
        ...
        for depth in range(1, max_depth + 1):
            ...
            db.insert_lpips_metric(curr["id"], 
                                   prev["id"], 
                                   int(curr["user_id"]),  # <-- cast here
                                   int(chat_id),
                                   int(depth),
                                   float(lpips_val))
            
            # print(f"LPIPS computed for depth {depth}: {lpips_val:.4f}")


# def recompute_lpips_for_chat(chat_id):
#     with Database() as db:
#         images = db.fetch_images_by_chat(chat_id, pandas=True, include_depth=True)
#         images = images.sort_values("depth")
#         prompts = db.fetch_prompts_by_chat(chat_id)

#         for depth in range(1, images["depth"].max() + 1):
#             current = images[images["depth"] == depth]
#             previous = images[images["depth"] == depth - 1]
#             if current.empty or previous.empty:
#                 continue

#             curr_row = current[current["selected"] == 1]
#             prev_row = previous[previous["selected"] == 1]
...
#             # Check if this LPIPS record already exists
#             existing = db.fetch_lpips_by_chat(chat_id)
#             exists = ((existing["image_id"] == curr["id"]) & (existing["previous_image_id"] == prev["id"])).any()
#             if exists:
#                 continue

#             score = get_or_compute_lpips(curr["id"], curr["path"], prev["path"])
#             db.insert_lpips_metric(
#                 image_id=curr["id"],
#                 previous_image_id=prev["id"],
