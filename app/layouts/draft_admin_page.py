...
def update_guidance_plot(dashboard_data):
    ...
    df = df.sort_values(by=['chat_id', 'depth'])
...
def update_novelty_amplitude(dashboard_data):
    if dashboard_data is None or not dashboard_data.get("data_loaded"):
        raise PreventUpdate
    ...
    bert_df = bert_df.sort_values(by=['chat_id', 'depth'])
    lpips_df = lpips_df.sort_values(by=['chat_id', 'depth'])
...
def update_functionality_usage(dashboard_data):
    if dashboard_data is None or not dashboard_data.get("data_loaded"):
        raise PreventUpdate
...
def update_keywords_graph(dashboard_data):
    if dashboard_data is None or not dashboard_data.get("data_loaded"):
        raise PreventUpdate
